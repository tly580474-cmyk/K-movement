...
和声按 4/4 小节规划，使用最近转位、共同音保持和有限声部移动连接相邻和弦；四声部评分还会避免窄音域堆叠、过密相邻音和平行五/八度。A/A′ 以半终止形成“逗号”，A′ 末属和弦解决到 B 段新调式主和弦，B 末以伪终止制造意外，只有最终 A″ 使用 V→I 正格终止。强拍旋律优先使用当前和弦音，趋势状态会自动选择大小调倾向。所有发声音符保留对应的 `candleIndex`，相同请求仍会得到确定性的乐章 ID 与音符结果。

MIDI 导出采用 Type 1 多轨格式，包含指挥轨、主旋律、自动和声和低音轨，可在常见 DAW 或制谱软件中分别编辑。

生成风格不仅是展示标签：轻爵士使用七和弦、统一网格 swing comping 与下方半音趋近的错位 walking bass；Lo-fi 强制使用可 swing 的切分节奏；史诗风扩大力度、音域、使用低音级进，并加入段首 crash、逐小节定音鼓和段末 roll；国风使用双音五声音阶分解和声；极简氛围使用踏板低音和长延音；摇滚使用根音—五度—八度低音，过门规模与力度随段落层级和行情活动度变化。最终小节由 4 拍主音、持续和弦和踏板低音共同收束。低活跃小节会让旋律、和声、低音及鼓组同步呼吸。浏览器播放器加入风格化混响和钢琴踏板式释放，采样失败时使用柔和包络降级。
