...
当前工作区只有产品描述和界面参考图，尚无工程代码。参考图为 1680×945 的深色桌面数据工作台，主要包含：
...
2. 建立设计令牌与 AppShell；
3. 完成左侧导航、顶部标题和主网格；
4. 完成 K 线面板、旋律面板、右侧信息栏的静态样式；
5. 使用 Mock 数据完成基础图表；
6. 建立视觉回归截图；
7. 评审视觉还原后，再进入播放器和生成交互。

第一迭代不连接 MySQL、不实现 MIDI 生成，目标是把产品骨架、视觉语言和组件边界一次确定下来。
